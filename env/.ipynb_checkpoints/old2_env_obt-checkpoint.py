...
class CustomObstacleTowerEnv(gym.Env):
    ...
    def __init__(
        ...
        if self.retro or self.mode == 'retro_large':
            image_space_max = 255
    ...
    def _make_observation(self, visual_arr, reward, done, step_result):
        ...
        if self.uint8_visual:
            visual_arr = (255.0 * visual_arr).astype(np.uint8)

        if self.retro:
            # Resize if needed
            visual_arr = self._resize_observation(visual_arr, self._image_shape)
            # If you want to add overlay info, you can pass step_result.obs[1] if there's vector data
            # or skip it if you have no vector data.
            # For now, let's assume you do not have that vector data from the new API
            default_observation = visual_arr
        else:
            # If you want a tuple (image, keys, time_remaining)
            # you'd need to parse the second obs array from step_result, if it exists.
            # e.g. vector_obs = step_result.obs[1][agent_id] if you do have that
            # We'll stub it out:
            keys = 0
            time_remaining = 0
            default_observation = (visual_arr, keys, time_remaining)
